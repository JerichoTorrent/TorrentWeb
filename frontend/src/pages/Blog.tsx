...
const Blog: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0e0e10] to-[#1a1a1e] text-white">
      ...
      <div className="w-full px-4 sm:px-8 md:px-12 lg:px-20 xl:px-32 max-w-7xl mx-auto py-12">
        <h1 className="text-4xl font-bold text-yellow-400 mb-10 text-center pt-2 md:mt-16">
          Blog
        </h1>
            ...
            <div className="bg-[#111] border border-gray-700 rounded-lg p-4">
              ...
              <ul className="text-sm space-y-2 pl-1 list-disc marker:text-purple-500 text-purple-400">
                {posts.slice(0, 10).map((post) => (
                  <li key={post.slug}>
                    <Link to={`/blog/${post.slug}`} className="hover:text-purple-300 transition">
                      {post.metadata.title}
                    </Link>
                  </li>
                ))}
